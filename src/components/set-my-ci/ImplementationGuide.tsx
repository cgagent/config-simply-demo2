...
const ImplementationGuide: React.FC<ImplementationGuideProps> = ({
  ...
  const steps = selectedCI === 'github' 
    ? [
        ...
        },
        {
          ...
          icon: <GitBranchPlus className="h-8 w-8 text-blue-600" />
        },
        {
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-blue-200 shadow-md">
      <h2 className="text-2xl font-bold mb-4 text-blue-900">Step 4: Implementation Guide</h2>
      ...
      <div className="space-y-8 mb-10">
        {steps.map((step, index) => (
          <div key={index} className="flex items-start">
            <div className="flex-shrink-0 bg-blue-100 p-4 rounded-full mr-5 border border-blue-300 shadow-sm">
              {step.icon}
            </div>
            <div>
              <h3 className="font-semibold text-xl text-blue-800">{index + 1}. {step.title}</h3>
              <p className="text-base text-blue-700 mt-2">{step.description}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-8 flex justify-end">
        <Button onClick={onFinish} size="lg" className="text-base px-8 py-6 bg-gradient-to-r from-blue-700 to-blue-500 hover:from-blue-600 hover:to-blue-400 shadow-lg">
          Finish
        </Button>
      </div>
    </div>
